...
page::setup('/local/alphatrade/community.php', 'community', get_string('community', 'local_alphatrade'));
...
$spaces = [
    'qaforum' => ['icon' => 'ph-chats-circle', 'key' => 'community_qa'],
...
foreach ($spaces as $setting => $space) {
    ...
    if ($card) {
        $cards[] = array_merge($card, [
            ...
            'rule' => $setting === 'analysisforum' ? get_string('nosignalsrule', 'local_alphatrade') : '',
        ]);
    }
}
